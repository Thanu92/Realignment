...
fish_4521 <- read.table("Fish_Realigned_multigene.phy")

# Install from CRAN
#install.packages("tidyverse")
library(tidyverse)
#Rename columns
colnames(fish_4521) <- c("species_name","seq")

#New dataframe by removing first row as it consists of number of species and the multigene seq length
fish_multigene <- fish_4521[-1,]

head(fish_multigene)

#Extract the range of COI gene to be used in EPA

fishsamplefull_co1 <- substr(fish_multigene$seq,2761,3441)
#Check the class
...
dat2phylip(fishsamplefull_co1_with_speciesname,outfile= "fishsample4521_OutG_co1_with_speciesname.phy")
#Upto here okay
4521*(80/100)#3617
...
fishsample80F <- fishsamples(fish_multigene,3617)
dim(fishsample80F)
