...
#In Stratified folder
Correct_position<- read.csv("Strati_SWtools_new_extened_ART.csv")
head(Correct_position,20)
...
#From the plot above, as all the points fall approximately along the reference line except in 60% EPAng and RAxMLEPA, can we assume normality???No we can't. The data violates the normality.

#Assumption 2. Homogeneity of Variances
#Use Levene's test to check if variances are equal across groups.
# Load necessary library
library(car)

# Perform Levene's Test
leveneTest(Correct_percentage ~ SW_Tool * Sample_Type, data = Correct_position)


#All the three assumptions are violates. So, need to use a non-parametric test for the calculation.
# #Friedman test
# # Reshape the data into a long format
# long_data <- Correct_position %>%
#   group_by(id, Sample_Type, SW_Tool) %>%
#   summarize(Correct_percentage = mean(Correct_percentage)) %>%
#   spread(Sample_Type, Correct_percentage)
# 
# # Perform the Friedman test
# friedman_result <- friedman.test(as.matrix(long_data[, -1]))
# # Print the result
...
# library(PMCMRplus)
#Step 1: Using ART (Aligned Rank Transform)
#First, ensure your data is in the correct format and apply the ART method.
# Ensure the data is in long format and factors are appropriately set
long_data <- Correct_position %>%
  mutate(SW_Tool = factor(SW_Tool), Sample_Type = factor(Sample_Type), id = factor(id))
library(ARTool)
#Checking without random effect
# # Only fixed effects should be used with art
# art_model <- art(Correct_percentage ~ SW_Tool * Sample_Type, data = long_data)
# #summary(art_model)
# anova_art_model <- anova(art_model)
# summary(anova_art_model)
# anova_art_model 
# Use the aov object from the ART model
# lm_model <- artlm(art_model, "SW_Tool:Sample_Type")
# plot(lm_model)
# library(emmeans)
# # Perform post-hoc tests
# # Obtain estimated marginal means
# emmeans_model <- emmeans(lm_model, ~ SW_Tool* Sample_Type)
# 
...
# Fit the ART model
art_model <- art(Correct_percentage ~ SW_Tool * Sample_Type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)

#Posthoc
library(dplyr)   # for %>% pipe
art.con(art_model , "SW_Tool:Sample_Type") %>%   # run ART-C for SW_Tool * Sample_Type
...
#-----------------
#In Stratified folder
Correct_position<- read.csv("RAxML_R_S_correct_extended_ART.csv")
head(Correct_position,20)
...
Sample <- factor(Correct_position$Sample)
sample_type <- factor(Correct_position$sample_type)
correct_placement_percentage <- factor(Correct_position$correct_placement_percentage)
res.aov <- anova_test(
  data = Correct_position, dv = correct_placement_percentage , wid = id,
  between = c(Sample, sample_type)
)
get_anova_table(res.aov)# this automatically check the homogeneity of groups using mauchley's test
...
# Ensure the data is in long format and factors are appropriately set
long_data <- Correct_position %>%
  mutate(Sample = factor(Sample), sample_type = factor(sample_type), id = factor(id))
library(ARTool)


# Fit the ART model
art_model <- art(correct_placement_percentage ~ Sample * sample_type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)
...
#Posthoc
library(dplyr)   # for %>% pipe
art.con(art_model , "Sample:sample_type") %>%   # run ART-C for SW_Tool * Sample_Type
  summary() %>%  # add significance stars to the output
  mutate(sig. = symnum(p.value, corr=FALSE, na=FALSE,
                       cutpoints = c(0, .001, .01, .05, .10, 1),
                       symbols = c("***", "**", "*", ".", " ")))
...
#In Stratified folder
Correct_position<- read.csv("S_Genus_SWTools_extended_ART.csv")
head(Correct_position,20)
...
#Create box plots of the Correct_percentage colored by SW_Tool groups:
bxp <- ggboxplot(
  Correct_position, x = "Sample_Type", y = "Genus_position",
  color = "SW_Tool", palette = "jco"
)
bxp
...
ggqqplot(Correct_position, "Genus_position", ggtheme = theme_bw()) +
  facet_grid(Sample_Type ~ SW_Tool, labeller = "label_both")
#From the plot above, as all the points fall approximately along the reference line except in 60% EPAng and RAxMLEPA, can we assume normality???No we can't. The data violates the normality.
...
# Perform Levene's Test
leveneTest(Genus_position ~ SW_Tool * Sample_Type, data = Correct_position)
...
library(ARTool)
# Fit the ART model
art_model <- art(Genus_position ~ SW_Tool * Sample_Type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)
...
Correct_position<- read.csv("Random_correct_position_SW_tools_extended_ART.csv")
head(Correct_position,20)
...
#Read the stratified sw tools for 5 different sampling types csv file
Genus<- read.csv("R_Genus_SWTools_extended_ART.csv")
head(Genus)
names(Genus)
#Summary statistics
Genus %>%
...
#-------------Genus_R_S for RAxML-EPA
#In Stratified folder
Correct_position<- read.csv("RAxML_R_S_Genus_extended_ART.csv")
head(Correct_position,20)
...
# # Perform Levene's Test
#leveneTest(Genus_position ~ Sample * Sample_Type, data = Correct_position)
# Ensure the data is in long format and factors are appropriately set
long_data <- Correct_position %>%
  mutate(Sample = factor(Sample), Sample_Type = factor(Sample_Type), id = factor(id))
res.aov <- anova_test(
  data = long_data, dv = Genus_position , wid = id,
  between = c(Sample,Sample_Type)
)
get_anova_table(res.aov)
...
art_model <- art(Genus_position ~ Sample * Sample_Type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)
...
#Read the stratified sw tools for 5 different sampling types csv file
RF<- read.csv("RF_Strati_SWtools_extended_Within.csv")
head(RF,60)
...
long_data <- RF %>%
  mutate(SW_Tool = factor(SW_Tool), Sample_Type = factor(Sample_Type), id = factor(id))
# Fit the ART model
art_model <- art(RF_distance ~ SW_Tool * Sample_Type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)
...
RF<- read.csv("RAxML_R_S_RF_extended_ART.csv")
head(RF,60)
...
ggqqplot(RF, "RF_dist", ggtheme = theme_bw()) +
  facet_grid(sample_type ~ Sample, labeller = "label_both")
#From the plot above, as all the points fall approximately along the reference line except in 60% EPAng and RAxMLEPA, can we assume normality???yes
#Computation
...
long_data <- RF %>%
  mutate(Sample = factor(Sample), sample_type = factor(sample_type), id = factor(id))
# Fit the ART model
art_model <- art(RF_dist ~ Sample * sample_type + Error(id), data = long_data)
summary(art_model)
# Perform ANOVA on the ART-transformed data
anova_art_model <- anova(art_model)
print(anova_art_model)
...
#checking-----
subject_id <- rep(1:10, each=10)
SW_Tool <- rep(rep(c("RAxMLEPA", "EPAng"), each=5), times=10)
Sample_Type <- rep(c("0.2S", "0.4S", "0.6S", "0.8S", "0.99S"), times=20)
Correct_percentage <- rnorm(100, mean=50, sd=10)
# Create the data frame
your_data <- data.frame(Subject=subject_id, SW_Tool=SW_Tool, Sample_Type=Sample_Type, Correct_percentage=Correct_percentage)
# Ensure the data is in long format and factors are appropriately set
long_data <- your_data %>%
  mutate(SW_Tool = factor(SW_Tool), Sample_Type = factor(Sample_Type), Subject = factor(Subject))

# Fit the ART model
art_model <- art(Correct_percentage ~ SW_Tool * Sample_Type + (1|Subject), data = long_data)
summary(art_model)
#-----------
...
# Perform post-hoc tests
posthoc <- emmeans(art_model, pairwise ~ SW_Tool * Sample_Type)
print(posthoc)
